...
class beluxeModel extends beluxe
{
	...
	function _arrangeCategory(&$p_lst, $list, $depth)
	{
		...
		foreach($list as $key => $val) {
			...
			$obj->navigation = (object)array(
				...
				'list_count' => (int) ($navi[2] ? $navi[2] : $p_lst[$t_prsrl]->navigation->list_count),
				'page_count' => (int) ($navi[3] ? $navi[3] : $p_lst[$t_prsrl]->navigation->page_count),
				'clist_count' => (int) ($navi[4] ? $navi[4] : $p_lst[$t_prsrl]->navigation->clist_count)
			);
		...
		if(!$GLOBALS['BELUXE_CATEGORY_LIST'][$a_modsrl])
		{
			...
			if($oModIfo->module_srl)
			{
				if($oModIfo->category_default_type){
					...
					$oModIfo->default_category_title = $oModIfo->category_default_title;
				}
	...
	function getColumnInfo($a_modsrl)
	{
		...
		if(!$GLOBALS['BELUXE_COLUMN_LIST'][$a_modsrl])
		{
			$tmp = __XEFM_CACHE__.sprintf('%s.clm.php', $a_modsrl);
			if(!file_exists($tmp))
			{
				require_once(__XEFM_PATH__.'classes.cache.php');
				BeluxeCache::columnConfigList($a_modsrl);
			}
			@include($tmp);
			$column_info = is_array($column_info) ? $column_info : array();
			$GLOBALS['BELUXE_COLUMN_LIST'][$a_modsrl] = $column_info;
		}

		return $GLOBALS['BELUXE_COLUMN_LIST'][$a_modsrl];
	}
	...
	function getNavigationList($obj, $a_ectnotice = FALSE, $a_loadextra = TRUE, $a_collst = array())
	{
		...
		$out = $cmDocument->getDocumentList($obj, $a_ectnotice, $a_loadextra);
		...
		{
			$obj->page = $is_prev;
			$outtmp1 = $cmDocument->getDocumentList($obj, $a_ectnotice, $a_loadextra);
		...
		{
			$obj->page = $is_next;
			$outtmp3 = $cmDocument->getDocumentList($obj, $a_ectnotice, $a_loadextra);
			if(count($outtmp3->data)) $outtmp2 = array_merge((array)$outtmp2, (array)$outtmp3->data);
		}

		$idx = 0;
		foreach($outtmp2 as $val)
		{
		...
		$out->total_page = 0;
		$out->page_navigation = new PageHandler($out->total_count, 0, $idx - $is_prev, 0);
		$out->page = $page;
	...
	function getBestList($a_modsrl = 0)
	{
		...
		// cache controll 자주 바껴서 캐시까지 쓸 필요없을것 같다.
	...
	function getCommentByMemberSrl($a_docsrl, $a_mbrsrl, $a_collst = array())
	{
		...
		{
			$object_key = 'object:' . $a_docsrl . '_member_srl:' . $a_mbrsrl;
			$cache_key = $oCacheNew->getGroupKey('commentList_' . $a_docsrl, $object_key);
			$out = $oCacheNew->get($cache_key);
		}
		...
		if(!$out)
		{
			...
			$out->data = $this->_setCommentItem($out->data);
			//insert in cache
			if($oCacheNew->isSupport()) $oCacheNew->put($cache_key, $out);
		}
	...
	function isBlind($a_docsrl, $a_type = 'doc')
	{
		...
		{
			if($a_type == 'cmt')
				$t_vals = $this->_getCommentColumns($a_docsrl, array('blamed_count'));
			else
				$t_vals = $this->_getDocumentColumns($a_docsrl, array('blamed_count'));
