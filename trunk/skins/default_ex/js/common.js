...
jQuery(function($)
{
	...
	.each(function()
	{
		...
		if(iscolm)
		{
			var $p = $('#siHrm:not(.noheader):eq(0),#siLst:not(.noheader):eq(0)'),
	...
	$('tr.scMRp .scMRpWrt button')
	.click(function()
	{
		var $p = $(this).closest('tr.scMRp'),
			nik = $('.scMRpWrt input[data-name=nick_name]', $p).val(),
			pas = $('.scMRpWrt input[data-name=password]', $p).val(),
			txt = $('.scMRpWrt input[data-name=content]', $p).val(),
			srl = $(this).attr('data-srl');
		var $f = $('#siMemoRpFrm'),
			ru = $('input[name=success_return_url]', $f).val();
		$('input[name=nick_name]', $f).val(nik);
		$('input[name=password]', $f).val(pas);
		$('input[name=content]', $f).val(txt);
		$('input[name=document_srl]', $f).val(srl);
		$('input[name=success_return_url]', $f).val(ru.setQuery('document_no',srl));
		$f.submit();
		return false;
	});

	$('form .scGotoLoginPage')
	.focus(function()
	{
		if($(this).data('focus') || 0) return;
		$(this).data('focus',1);
		if(!confirm('Do you want to go the login page?')) return;
		location.href = current_url.setQuery('act','dispMemberLoginForm');
	})
	.blur(function()
	{
		$(this).data('focus','');
	...
	.load(function()
	{
		...
		$('#siDoc a[name^=document][data-scroll=true]').last().parent().scrollIntoView();
